...
#
#  · Neutron Clang diye compile
#  · Output: AnyKernel3 zip
# ════════════════════════════════════════════════════════════════
set -e
...
if [ -f "$DTBO" ]; then
    echo "  dtbo.img     : $(du -h "$DTBO" | cut -f1)"
fi
...
# AnyKernel3 directory clean up (purana kernel files hata)
cd "$AK3_DIR"
